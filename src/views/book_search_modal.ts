...
export class BookSearchModal extends Modal {
  ...
  constructor(
    ...
    this.serviceProvider = factoryServiceProvider(plugin.settings, plugin.serviceProviderOverride);
  }
  ...
  onOpen(): void {
    ...
    const service = this.plugin.serviceProviderOverride || this.plugin.settings.serviceProvider;
    contentEl.createEl('h2', { text: `Search Book (${service})` });

    if (service === ServiceProvider.google && this.plugin.settings.askForLocale) this.renderSelectLocale();
