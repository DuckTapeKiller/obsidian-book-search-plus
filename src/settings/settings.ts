...
export class BookSearchSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    const hideServiceProviderExtraSettingButton = () => {
      if (serviceProviderExtraSettingButton) serviceProviderExtraSettingButton.addClass('book-search-plugin__hide');
    };
    const showServiceProviderExtraSettingButton = () => {
      if (serviceProviderExtraSettingButton) serviceProviderExtraSettingButton.removeClass('book-search-plugin__hide');
    };
    const hideServiceProviderExtraSettingDropdown = () => {
      if (preferredLocaleDropdownSetting) preferredLocaleDropdownSetting.settingEl.addClass('book-search-plugin__hide');
    };
    ...
    const showCalibreSettings = () => {
      if (calibreServerUrlSetting) calibreServerUrlSetting.settingEl.removeClass('book-search-plugin__hide');
      if (calibreLibraryIdSetting) calibreLibraryIdSetting.settingEl.removeClass('book-search-plugin__hide');
      if (calibreSettingsHeader) calibreSettingsHeader.settingEl.removeClass('book-search-plugin__hide');
    };
    const hideCalibreSettings = () => {
      if (calibreServerUrlSetting) calibreServerUrlSetting.settingEl.addClass('book-search-plugin__hide');
      if (calibreLibraryIdSetting) calibreLibraryIdSetting.settingEl.addClass('book-search-plugin__hide');
      if (calibreSettingsHeader) calibreSettingsHeader.settingEl.addClass('book-search-plugin__hide');
    };
    ...
    new Setting(containerEl)
      ...
      .addDropdown(dropDown => {
        ...
        dropDown.addOption(ServiceProvider.goodreads, `${ServiceProvider.goodreads} (Scraping)`);
        dropDown.addOption(ServiceProvider.calibre, `${ServiceProvider.calibre} (Local Server)`);
    ...
    calibreLibraryIdSetting = new Setting(containerEl)
      ...
      .setDesc('Enter the Library ID (default: calibre). This is usually the folder name of your library.')
      .addText(text =>
        text
